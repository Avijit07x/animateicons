...
<Callout type="note">
	The ref handle only works in Client Components. Add `"use client"` to the file
	that calls `startAnimation()`.
</Callout>
...
<Callout type="note">
	Animations respect the OS-level **Reduce Motion** preference, so no extra
	setup is required.
</Callout>
